...
public class FileListServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			String uploadFilePath2 = this.getServletContext().getRealPath("/WEB-INF/path.txt");
			FileReader reader = new FileReader(uploadFilePath2);
			BufferedReader br = new BufferedReader(reader);
			String path1 = br.readLine();

			String endpath = (String) request.getParameter("endpath");// 这个是用来点超链接返回某目录的
			System.out.println("哈哈哈哈endpath是：" + endpath);
			System.out.println("哈哈哈哈path1是：" + path1);

			String newpath = "";
			if (endpath != null) {
				newpath = path1.substring(0, path1.indexOf(endpath)) + endpath;
				path = newpath;
			}
			System.out.println("哈哈哈哈newpath是：" + newpath);
			...
			// 将当前path存入path.txt文档
			String uploadFilePath1 = this.getServletContext().getRealPath("/WEB-INF");
			File fff = new File(uploadFilePath1 + "/path.txt");
			if (!fff.exists()) {
				fff.createNewFile();
			}
